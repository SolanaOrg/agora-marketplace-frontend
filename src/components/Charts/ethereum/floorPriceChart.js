    ...
    series: [
      {
        ...
        data: getFloorData?.map((chart) => {
          var temp_chart = Number(chart).toFixed(4);
          const res_chart = temp_chart.slice(-1) === '0'
            ? temp_chart.slice(0, -1)
            : temp_chart;
          return res_chart;
        })
      },
      ...
      <div className='floorChart-control' >
        {
          getFloorData ?
            <ReactApexChart
              ...
              type="area"
              width={`100%`}
              height={isLargeDesktop ? 350 : 250}
            />
            :
            <SkeletonTheme baseColor="#202020" highlightColor="#444">
              <p style={{ width: `90%`, margin: `0 auto` }} >
                <Skeleton count={1} style={{ minHeight: isLargeDesktop ? `350px` : `250px` }} />
              </p>
            </SkeletonTheme>
        }
      </div>
