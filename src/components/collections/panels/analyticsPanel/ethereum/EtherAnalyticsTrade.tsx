...
const EtherAnalyticsTrade = (props: any) => {
  const { symbol } = props;
      ...
      async () => {
        try {
          ...
          const get_recent_trade: any = await commonService({
            method: `GET`,
            route: `${MARKETPLACES_ETH_API.GET_COLLECTION}/${symbol}/recent_trades`
          })
          console.log('get_recent_trade', get_recent_trade)
                    ...
                    return (
                      <tr key={idx} >
                        ...
                        <td>
                          <div className="d-flex align-items-center">
                            <Icons name={97} />
                            <p className="mx-2 sand-a2kewade">{item.price} ETH</p>
                          </div>
                          <div>
                            <h5 className='anbsdksa-oakew'>(${(item.price * ethToUsd).toFixed(2)})</h5>
                          </div>
                        </td>
                      </tr>
                    )
...
export default EtherAnalyticsTrade