  ...
  const handleNftBuy = async () => {
    ...
    try {
      ...
      if (res?.txid) {
        ...
        setOwnerByAddress(anchorWallet?.publicKey.toBase58())
        setNftInfo({ ...nftInfo, status: 0 })
      ...
      async () => {
        try {
          ...
          const get_all_attributes: any = await commonService({
            method: `get`,
            route: `${MARKETPLACES_API.GET_COLLECTION_DATA}${name}/attributes`
          })

          let attributes = []
          for (let i = 0; i < get_all_attributes.attributes.length; i++) {
            for (let j = 0; j < getMetadata.attributes.length; j++) {
              if ((get_all_attributes.attributes[i].type === getMetadata.attributes[j].trait_type) && (get_all_attributes.attributes[i].value === getMetadata.attributes[j].value)) {
          ...
          if (!listItemResult.data) {
            ...
            setNftInfo({ ...getMetadata, attributes: attributes })
          }

          if (listItemResult.status) {
            setNftInfo({ ...listItemResult, attributes: attributes })
  ...
  return (
    <Layout>
      ...
      <div className="nft-layout">
        <Link to={`/collection/solana/${name}?activeTab=items`} style={{ textDecoration: `none` }} className='back-btn' >
