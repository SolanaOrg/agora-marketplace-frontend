...
const NftEtherDetail = () => {
  ...
  useEffect(() => {
    if (storeData.nftItemClick === true) {
      window.scrollTo({
        top: 0,
      });
    }
  ...
  useEffect(() => {
    (
      async () => {
        ...
        const get_all_attributes: any = await commonService({
          method: `get`,
          route: `${MARKETPLACES_ETH_API.GET_COLLECTION}/${contractAddress}/attributes`
        })

        let attributes = []
        if (get_nftInfo?.nft?.attributes) {
          for (let i = 0; i < get_all_attributes.attributes.length; i++) {
            for (let j = 0; j < get_nftInfo?.nft?.attributes.length; j++) {
              if ((get_all_attributes.attributes[i].type === get_nftInfo?.nft?.attributes[j].trait_type) && (get_all_attributes.attributes[i].value === get_nftInfo?.nft?.attributes[j].value)) {
                attributes.push(get_all_attributes.attributes[i])
              }
            }
          }
        } else {
          for (let i = 0; i < get_all_attributes.attributes.length; i++) {
            for (let j = 0; j < JSON.parse(get_nftInfo?.metadata)?.attributes.length; j++) {
              if ((get_all_attributes.attributes[i].type === JSON.parse(get_nftInfo?.metadata)?.attributes[j].trait_type) && (get_all_attributes.attributes[i].value === JSON.parse(get_nftInfo?.metadata)?.attributes[j].value)) {
                attributes.push(get_all_attributes.attributes[i])
              }
            }
          }
  ...
  return (
    <Layout>
      ...
      <div className="nft-layout">
        <Link to={`/collection/ether/${contractAddress}?activeTab=items`} style={{ textDecoration: `none` }} className='back-btn' >
...

                {
                  !nftInfo?.image ? <SkeletonTheme baseColor="#202020" highlightColor="#444">
                    ...
                    <img src={nftInfo?.image} style={{
                      width: `100%`, minHeight: `500px`, borderRadius: `10px`
                    }} alt="NFT"
                      onError={(e) => {
                      ...
                      <>
                        ...
                        <div className="list-bottom" >
                          <div className="title-group" >
                            ...
                            {
                              nftInfo?.contract_type === 'ERC1155' && <div className="nft-count" >
